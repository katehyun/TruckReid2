...
sub_all <- TargetTable9[,1:4]
sub_all <- cbind(sub_all,TargetTable9[,6], TargetTable9[,threshold])
sub_all <- cbind( sub_all,      
                      ...
                      Upheader_new[ match( sub_all[,6], as.numeric(Upheader_new[,13])),13:20] ,
...
sub_matching <- subset(sub_all , as.numeric (sub_all [,5]) == as.numeric(sub_all [,6]) &
                         as.numeric (sub_all[,5]) != 999)
sub_nonmatching <- subset(sub_all, as.numeric (sub_all[,5]) != as.numeric( sub_all[,6]) )
...
colnames(sub_all_diff)[9:16] <- c("mlength", "mgvw", "max12sp", "max23sp", "max34sp", "max45sp", 
                                     "mduration", "mutc")
colnames(sub_all_diff)[17:24] <- c("nmlength", "nmgvw", "nmax12sp", "nmax23sp", "nmax34sp", "nmax45sp", 
                                     "nmduration", "nmutc")

q <- 8
sub_all[,length(sub_all[1,])+1] <- sub_all_diff[,q+1] * sub_all_diff[,q+2]* sub_all_diff[,q+3] * sub_all_diff[,q+4]* sub_all_diff[,q+5]* sub_all_diff[,q+6]* sub_all_diff[,q+7]* sub_all_diff[,q+8]

qq <- 16
sub_all[,length(sub_all[1,])+1] <- sub_all_diff[,qq+1] * sub_all_diff[,qq+2]* sub_all_diff[,qq+3] * sub_all_diff[,qq+4]* sub_all_diff[,qq+5]* sub_all_diff[,qq+6]* sub_all_diff[,qq+7]* sub_all_diff[,qq+8]
